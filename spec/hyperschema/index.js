...
const encoding2 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.appId) c.string.preencode(state, m.appId)
    if (version >= 2 && m.extra) c.buffer.preencode(state, m.extra)
  },
  encode(state, m) {
    const flags = (version >= 2 && m.appId ? 1 : 0) | (version >= 2 && m.extra ? 2 : 0)

    encoding2_0.encode(state, m.block)
    encoding2_1.encode(state, m.destination)
    c.uint.encode(state, flags)

    if (version >= 2 && m.appId) c.string.encode(state, m.appId)
    if (version >= 2 && m.extra) c.buffer.encode(state, m.extra)
  },
  decode(state) {
    ...
    return {
      block: r0,
      destination: r1,
      appId: version >= 2 && (flags & 1) !== 0 ? c.string.decode(state) : null,
      extra: version >= 2 && (flags & 2) !== 0 ? c.buffer.decode(state) : null
    }
  }
}
